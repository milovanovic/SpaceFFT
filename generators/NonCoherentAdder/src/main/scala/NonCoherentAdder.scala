...
import chisel3._ 
import chisel3.util.{Queue, ShiftRegister, log2Ceil}
...
class NonCoherentAdder[T <: Data: Real : BinaryRepresentation] (params: NonCoherentAdderParams[T]) extends LazyModule()(Parameters.empty) {
  val streamNode = AXI4StreamNexusNode(
    masterFn = (ms: Seq[AXI4StreamMasterPortParameters]) => AXI4StreamMasterPortParameters(ms.map(_.masters).reduce(_ ++ _)),
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    def sumRecursive[T <: Data: Real : BinaryRepresentation](in: Seq[T]): T = {
      ...
      else sumRecursive(in.grouped(2).map{
        case data: Seq[T] => {
          DspContext.alter(DspContext.current.copy(numAddPipes = params.numAddPipes, trimType = params.trimType, overflowType = Grow)) {
    ...
    val sumOfInputs = (params.proto match {
      case fp: FixedPoint => {
        if (fp.binaryPoint.get >= log2Ceil(ins.length)) {
          DspContext.withTrimType (params.trimType) { sumRecursive(ins.map{case (in) => in.bits.data.asTypeOf(params.proto)}).trimBinary(fp.binaryPoint.get - log2Ceil(ins.length)) }
        }
        else {
          if (params.verbose) println(s"Trimming was not possible, input BP width is: ${fp.binaryPoint.get}, data should be trimmed by ${log2Ceil(ins.length)} bits.")
    ...
    if (params.numAddPipes > 0) {
      ...
      dataQueue.io.enq.bits.data := BinaryRepresentation[T].shr(sumOfInputs, sumOfInputs.getWidth - params.proto.getWidth).asTypeOf(out.head.bits.data)
      dataQueue.io.enq.bits.last := ShiftRegister(ins.map{case (in) => in.bits.last}.reduce((a, b) => a && b), latency, false.B, en = true.B)
      dataQueue.io.enq.valid     := ShiftRegister(ins.map{case (in) => in.fire()}.reduce((a, b) => a && b), latency, false.B, en = true.B)
      ...
      out.head.bits.data := BinaryRepresentation[T].shr(sumOfInputs, sumOfInputs.getWidth - params.proto.getWidth).asTypeOf(out.head.bits.data)
...
object NonCoherentAdderApp extends App
{
  ...
  (new ChiselStage).execute(Array("--target-dir", "verilog/NonCoherentAdder"), Seq(ChiselGeneratorAnnotation(() => lazyDut.module)))
}