...
case class SpaceFFTParameters[T <: Data: Real: BinaryRepresentation] (
  ...
  scopeParams  : Option[ScopeParamsAndAddresses[T]]
)
...
/* Scope parameters and addresses */
case class ScopeParamsAndAddresses[T <: Data: Real: BinaryRepresentation] (
  scopeParams : ScopeParameters[T]
)
...
class SpaceFFTScopeParams(rangeFFTSize: Int = 512, dopplerFFTSize: Int = 256, ddrType: DDRType = DDR3) {
  val params : SpaceFFTParameters[FixedPoint] = SpaceFFTParameters (
    // Range parameters
    lvds1DParams = Some(LVDSPHYParamsAndAddresses(
      lvdsphyParams = DataRXParams(
